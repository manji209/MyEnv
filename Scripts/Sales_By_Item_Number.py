...
# For month and year
sqlstring = """SELECT item_no, desc_1, desc_2, cust_no, ord_no, invc_no, invc_or_cred, qty, prc, ext_prc, invc_dat FROM dbo.IHSLIN00 
            WHERE sls_rep =? AND
            DATEPART(YEAR, invc_dat)=? 
            ORDER BY item_no"""

df = pd.read_sql(sqlstring,conn, params=(sales_rep, year))
df['item_no'] = df['item_no'].str.strip()

# Create a Pandas Excel writer using XlsxWriter as the engine.
fout_name = '../Out/LARRY_ITEMS_REPORT_' + str(year) + '.xlsx'

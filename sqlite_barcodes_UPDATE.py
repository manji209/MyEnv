...
try:
    ...
    # Rename df_barcodes['keyword_token'] to df_barcodes['item_no'] to do join later
...
sql_conn = sqlite3.connect('Out/barcodes.db')

df_join.to_sql('barcodes', sql_conn, if_exists='replace', index=False)

pd.read_sql('select * from barcodes', sql_conn)
